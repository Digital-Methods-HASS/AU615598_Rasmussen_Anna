...
ggplot(subset(gapminder, year == 1952), aes(gdpPercap, lifeExp, size = pop)) +
  geom_point() +
  scale_x_log10() #transforming the scale into log10

```
We see an interesting spread with an outlier to the right. Answer the following questions, please:

Q1. Why does it make sense to have a log10 scale on x axis?
    As the data covers a wide range of values, it seems useful to transform the x axis into a log10 scale in        order to make it a more manageable size.
Q2. What country is the richest in 1952 (far right on x axis)?
  ANSWER: Kuwait
  Figured out using the following code:
```{r}
gapminder %>% 
  filter(year=="1952") %>% #filtering out that I only use the data corresponding to year 1952
  select(country,gdpPercap) %>%  #Choose the relevant coloumns
  arrange(desc(gdpPercap)) #arrange them in descending order
```
...
Q3. Can you differentiate the continents by color and fix the axis labels?
  YES, I did it - shown in the above snippet
  
Q4. What are the five richest countries in the world in 2007?

```{r}
gapminder %>% 
  filter(year=="2007") %>% #filtering out that I only use the data corresponding to year 2007
  select(country,gdpPercap) %>%  #Choose the relevant coloumns
  arrange(desc(gdpPercap)) #arrange them in descending order
```
ANSWER: The five richest countries in 2007 measured in GDP per Capita is Norway, Kuwait, Singapore, US and Ireland.
...
```{r}
anim2 + labs(title = "Year: {frame_time}") #labs = labels
```
...
With the following code I disable the scientific notation in general:
...
How is the lifeexpectancy in Denmark changing over years?
